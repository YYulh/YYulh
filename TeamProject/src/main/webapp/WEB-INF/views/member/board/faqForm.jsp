...
<br><br><br>
			...
			<c:choose>
				...
				<c:otherwise>			
					<c:forEach var="vo" items="${list}"> <!-- model에 저장된 list에서 정보 출력 -->
						<input type="hidden" value ="${vo.faq_no}"  name = "no">
						<tr>
							<th>${vo.faq_no}</th>
							<th class = "title">
							<a href="${pageContext.request.contextPath}/member/board/faqContent?no=${vo.faq_no}">${vo.bo_name} &nbsp; ${vo.faq_title}</a></th>
							<th>${vo.mb_name}</th>							
							<td>${vo.faq_date}</td>
							<td>${vo.faq_view}</td>												
						</tr>
		...
		<div class="btn">
			<c:if test = "${login.getMb_id()=='admin01'}">
				<input type = "button" value = "글쓰기" onclick ="location.href='${pageContext.request.contextPath}/member/board/faqWrite?board_no=3'">
			</c:if>
		</div>
	</form>
		<!-- 페이징 처리 한페이지에 50개씩 -->
